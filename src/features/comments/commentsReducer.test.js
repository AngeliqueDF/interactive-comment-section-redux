...
describe("commentsReducer", () => {
	...
	test("Removes an already added decrement vote", () => {
		const state = [
			{
				id: 1,
				content:
	...
	test("Decrements the score by 2 when the comment was already incremented", () => {
		const state = [
			{
				id: 1,
				content:
		...
		const newState = commentsReducer(state, action);
		expect(newState[0].score).toEqual(8);
		expect(newState[0].voteGiven).toEqual("decrement");
	});
});
